...
export default function Home() {
  ...
  const handleRetirementCalc = async(e) => {
    ...
    // Display Results
    document.getElementById("calcresult1").style.display = 'block';
    document.getElementById("calcresult2").style.display = 'block';
  }
  ...
  const handlePortfolio = async(e) => {
    ...
    // Display Results
    document.getElementById("portresult1").style.display = 'block';
    document.getElementById("portresult2").style.display = 'block';
    document.getElementById("portresult3").style.display = 'block';
  }
  ...
  return (
    <div className={styles.page}>
        ...
        <div className={styles.retirementcalc}>
          ...
          <p id="calcresult1" className={styles.result}> You would need to have saved a total of ${retireResult.retire_amt} to retire comfortably {retireResult.retire_years} years from today (at the time of retirement). This amount is adjusted for inflation of {retireResult.inflate_rate}% based on the average annual inflation in the United States over the past {retireResult.time_period} years and reflects the future value, not the amount you need to have saved today</p>
          <p id="calcresult2" className={styles.result}> For context, the amount would be worth ${retireResult.retire_amt_raw} today </p>
        </div>
        ...
        <div className={styles.portfolioest}>
          ...
          <p id="portresult1" className={styles.result}> Your portfolio of {portResult.stocks} stocks would be valued at ${portResult.value} with a profit of ${portResult.profit}. </p>
          <p id="portresult2" className={styles.result}> For context, the average price of {portResult.ticker} during the time period was ${portResult.avg_price}, and the end price was ${portResult.end_price}. Based on daily dollar cost averaging, the absolute rate of return was {portResult.abs_return}% </p>
          <p id="portresult3" className={styles.result}> If you held onto the stocks until today, your portfolio value would have been: ${portResult.today_value} with a net growth of ${portResult.final_profit} which is {portResult.final_return}% of total growth with an annualized rate of return of {portResult.ann_return}%</p>
        </div>
